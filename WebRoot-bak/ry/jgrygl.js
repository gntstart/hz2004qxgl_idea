...
function saveJz(p){
	Ext.Msg.show({
	   ...
	   fn: function(btn,text){
	  		if(btn=="yes"){
	 			Ext.Msg.wait("正在保存兼职设置，请稍后...", "等待");
				
				Ext.Ajax.request({
					url:'zzjy.do?method=saveJzZzjg',
					method:'POST', 
					params:p,
...
Ext.onReady( function() {
	...
	/********************************人员查询和列表*******************************/
 	...
 	ryCommonStore = new Ext.data.JsonStore({
        ...
        id:'yhid',
        totalProperty:'totalCount',
        fields: ["yhid","uid","yhdlm","jyh","dwdm","yhxm","yhxb","yhzw","dlkl","yhmj","czmj","yhzt","gmsfhm","dwmc"],
        listeners:{
			loadexception:function(mystore,options,response,error){
				if(error){
					var json = Ext.decode(response.responseText);
	...
	ryCsm = new Ext.grid.CheckboxSelectionModel({singleSelect:false});
            	...
            	items:[{
	                ...
	                columnWidth:.5,
        	       	...
        	       	items: [{
								...
								anchor:'100%',
								xtype:'DictComboBox',
								dict:"VisionType=CS_YHMJ&def=4&ignore=false",
								fieldLabel:'用户密级'
						}]
				}
			]},{
	...
	var jgmxWin = null;
	var jgmxFs = null;
	var jgCommonStore = null;
	var saveorgid = null;
	
	function savezzjg(){
		if(!jgmxFs.getForm().isValid()){
			Ext.Msg.alert("提示","数据校验没有通过，请检查！");
			return;
		}
		
		Ext.Msg.wait("正在保存单位信息，请稍后...", "等待");
		
	    jgmxFs.getForm().submit({
	       	url:"util/upfile?className=zzjyManagerService&method=saveZzjg",
	       	method:'POST',
	       	success:function(form,action){
				Ext.Msg.hide();
				
				var x1 = Ext.get('file2').dom.value;
				var x2 = Ext.get('file3').dom.value;
				
	    		Ext.get('file2').dom.outerHTML=Ext.get('file2').dom.outerHTML;
	    		Ext.get('file3').dom.outerHTML=Ext.get('file3').dom.outerHTML;
	    		
	    		if(saveorgid==""){
					jgmxFs.getForm().reset();
	    		}else{
					if(action.result){
						if(x1!=""){
							document.getElementById('img2').src = "util/upfile?type=dwgz&id=" + saveorgid + "&_dc=" + new Date().getTime();
						}
						if(x2!=""){
							document.getElementById('img3').src = "util/upfile?type=jzy&id=" + saveorgid + "&_dc=" + new Date().getTime();
						}
					}
					//rymxWin.hide();
				}
				...
				Ext.MessageBox.alert(
					'提示', 
					action.result.message + "关闭对话框刷新页面！",
					function(btnType){
	...
	//修改单位的代码
	function editdw(orgid,organizeBm,organizePath){
		//orgid=""，表示新增机构，否则表示修改该机构
		jgmxFs.getForm().reset();
		var cmp = Ext.getCmp("organizeBm");

		if(orgid==""){
			Ext.getCmp("saveBtnJG").setDisabled(false);
			//Ext.getCmp("delBtnJG").setDisabled(true);

			cmp.setDisabled(false);
			cmp.allowBlank = false;
		
			//保存上级单位编码
			jgmxFs.getForm().setValues({ParentOrganizeBm:organizeBm});
			jgmxWin.setTitle("在[" + organizePath + "]下新增单位");
			saveorgid = "";
		}else{
			Ext.getCmp("saveBtnJG").setDisabled(true);
			//Ext.getCmp("delBtnJG").setDisabled(true);
			...
			jgmxWin.setTitle("修改单位[" + organizePath + "]");
		}
		
		jgmxWin.show();
		
		if(orgid!=""){
			document.getElementById('img2').src = "util/upfile?type=dwgz&id=" + orgid + "&_dc=" + new Date().getTime();
			document.getElementById('img3').src = "util/upfile?type=jzy&id=" + orgid + "&_dc=" + new Date().getTime();
			jgCommonStore.load({params:{start:0, limit:pageSize,orgid:orgid}})
		}else{
			document.getElementById('img2').src = "images/no_pic.gif";
			document.getElementById('img3').src = "images/no_pic.gif";
		}
		
		_initReloadDict('form2');
	}

	jgCommonStore = new Ext.data.JsonStore({
        url: 'zzjy.do?method=queryZzjg',
        root: 'list',
        id:'orgid',
        totalProperty:'totalCount',
        fields: [
        ...
        listeners:{
        	load:function(mystore,res){
        		...
        		if(res.length>0){
        			//alert(Ext.encode(res[0].data));
					Ext.getCmp("saveBtnJG").setDisabled(false);
					//Ext.getCmp("delBtnJG").setDisabled(false);
        			jgmxFs.getForm().setValues(res[0].data);
        		}
        	},
			loadexception:function(mystore,options,response,error){
				myMask.hide();
            	...
            	items:[{
	                ...
	                columnWidth:.33,
        	       	...
        	       	items: [{
						...
						id:'organizeBm',
						allowBlank:false,
						fieldLabel:'<span style="color:red">*</span>单位编码'
					}]
				},{
	                columnWidth:.34,
    	           	layout: 'form',
	                ...
	                columnWidth:.34,
        	       	...
        	       	items: [{
								...
								anchor:'100%',
								xtype:'DictTextField',
								dict:"vid=ssqh&tid=ssqhname&VisionType=XZQH&ignore=false",
								fieldLabel:'所属区划'
					}]
				},{
	                columnWidth:.33,
        	       	...
        	       	items: [{
								...
								anchor:'100%',
								xtype:'DictComboBox',
								dict:"VisionType=DWJB&def=4&ignore=false",
								fieldLabel:'单位级别'
						}]
				},{
	                columnWidth:.33,
        	       	...
        	       	items: [{
								...
								anchor:'100%',
								xtype:'DictComboBox',
								dict:"VisionType=YWXTFL&def=4&ignore=false",
								fieldLabel:'业务系统分类'
						}]
				},{
	                columnWidth:.34,
        	       	...
        	       	items: [{
								...
								anchor:'100%',
								xtype:'DictComboBox',
								dict:"VisionType=DWLX&def=4&ignore=false",
								fieldLabel:'单位类型'
						}]
				},{
	                columnWidth:.33,
	                height:120,
    	           	layout: 'form',
    	           	bodyStyle:'padding:5 0 0 35',
        	       	html:'&nbsp;单位公章：&nbsp;<img id="img2" src="" width=100 height=100></img>'
				},{
	                columnWidth:.33,
	                height:120,
    	           	layout: 'form',
    	           	bodyStyle:'padding:5 0 0 50',
        	       	html:'&nbsp;局长印：&nbsp;<img id="img3" src="" width=100 height=100></img>'
				},{
	                columnWidth:.34,
    	           	layout: 'form',
    	           	bodyStyle:'padding:5 0 0 0',
	                ...
	                columnWidth:1,
        	       	...
        	       	items: [{
						...
						id:'adminlbname',
						anchor:'100%',
						dict:"vid=adminlb&tid=adminlbname&VisionType=RY&mselect=true",
						name:'adminlbname',
						fieldLabel:'机构管理员'
					}]
				}
	...
	function deleteTDQX(){
    	...
    	function addTDQX(){
    		...
    		treeSelectWin.items.first().loadData(
    			"serviceName=zzjyManagerService&serviceMethod=queryDwxxRyTree&VisionType=RY&mselect=false&showchecked=true&postDetail=" + dw.substring(0,4),
    			function(idlist){
    				Ext.Msg.wait("正在保存，请稍后...", "等待");
	...
	/************************************兼职设置*********************************************/
 	var queryDwCommonStore = new Ext.data.JsonStore({
        url: 'zzjy.do?method=querySearchZzjg',
        root: 'list',
        id:'organizeBm',
        totalProperty:'totalCount',
        listeners:{
        	beforeload:function(store, options ){
        		Ext.Msg.wait("正在查询单位，请稍后...");
        	},
	...
	var jzColModel = new Ext.grid.ColumnModel([{
	        header: "兼职单位",
	        dataIndex: "jzOrganizeInfoName",	
	        sortable: true,
			width: 100,
			renderer:function(value){
				return value;
	...
	var queryDwColModel = new Ext.grid.ColumnModel([{
 	...
 	jzCommonStore = new Ext.data.JsonStore({
        url: 'zzjy.do?method=queryZzjy',
        root: 'list',
        id:'jzOrganizeInfoBm',
        totalProperty:'totalCount',
	...
	var jzGrid = new Ext.grid.GridPanel({
        store: jzCommonStore,
        region: 'center',
        view:new Ext.grid.GridView({
				forceFit:true,
	...
	var queryDwGrid = new Ext.grid.GridPanel({
		...
		listeners:{
			...
			rowdblclick:function(g, rowIndex, e){
				var r1 = g.store.getAt(rowIndex);
				var r = new jzCommonStore.reader.recordType({
					yhdlm:curRy.data.yhdlm,
					jzOrganizeInfoBm:r1.data.organizeBm,
					jzOrganizeInfoName:r1.data.organizePath},r1.data.organizeBm);
        		jzCommonStore.add(r);
			}
		},
        title:''
		...
		buttons:[
			new Ext.Button({
				...
				handler:function(){
					...
					queryDwCommonStore.load({params:{start:0, limit:1000}})
				}
			}),
			new Ext.Button({
				text:'保存兼职',
				...
				id:'saveJzDwBtn',
				handler:function(){
					var yhdlm = curRy.data.yhdlm;
					var jzOrganizeInfoBm = "";
					jzCommonStore.each(
						function(rec){
							//this是each中指定的作用域window对象
							jzOrganizeInfoBm += rec.data.jzOrganizeInfoBm + ",";
						}
					);
					saveJz({yhdlm:yhdlm,jzOrganizeInfoBm:jzOrganizeInfoBm});
				}
			})
		]
	});
	
	var jzWin = new Ext.Window({
		title:'兼职设置',
		closeAction:'hide',
		modal:true,
		width:850,
		...
		margins:'5',
		layout:'border',
		items:[jzGrid,queryDwGrid,dwQueryFs],
		listeners:{
			show:function(){
			}
		}
