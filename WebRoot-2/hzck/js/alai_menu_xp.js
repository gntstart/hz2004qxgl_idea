...
function alai_menu_xp(width,imagelist)
{
	...
	this.add=function(Text,ico,exeType,exeArg,target)
	{
		...
		if(Text.length > 9)
			col2.innerHTML=Text.substring(0,6)+"...";
		else
			col2.innerHTML=Text
		col2.title=Text;
		...
		{
			case "hide":
				item.execute=function(){menu.hideAll();}
				break;
			case "url":
	...
	this.show=function()
	{
		...
		menu.iframe2.style.height=m.innerHTML.toString().split("</TR>").length*23;
		//alert(menu.iframe2.style.width);
		menu.iframe2.style.width=160*2;
		menu.isShow=true;
		if(menu.bar!=null)
		{
...
function menu_bar_xp(top,left)
{
	...
	this.body.style.cssText="position:absolute;cursor:default;padding:2;background-color:#EFF3FE;height:25;z-index:5;font-size:10pt;color:black;top:"+top+";left:"+left
	...
	this.add=function(Text,menu)
	{
		...
		item.style.cssText="margin:0 7 0 3;padding:2 4 2 4;text-align:center;height:23;background-color:#EFF3FE;border:1 solid #EFF3FE"
		item.innerText=Text
		item.onmouseover=function()
		{
			this.style.border="1 solid #0B2565"
			this.style.backgroundColor="#B7BED3";
			if(chkShow()){for(var i=0;i<col_menu.length;i++)
				{col_menu[i].hide();menu.show(item);
					}}
		}
		item.onmouseout=function()
		{
