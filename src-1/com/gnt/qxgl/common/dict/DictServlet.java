...
import com.gnt.qxgl.common.SynchronizedJob;
...
public class DictServlet extends HttpServlet{
		...
		if(CommonUtil.isNotEmpty(serviceName) && CommonUtil.isNotEmpty(serviceMethod)){
			...
			try{
				...
				if(value!=null){
					...
					/*resp.setContentType("text/html;charset=UTF-8"); 
					resp.getWriter().print(jison);  //换成这个就好了
				...
				//20170407kqt 注释更换
				/*resp.setContentType("text/html;charset=UTF-8"); 
				resp.getWriter().print(gson.toJson(s));  //换成这个就好了
				resp.getWriter().close();*/
			}
	...
	public void init() throws ServletException {
		...
		try{
			//String conf = System.getProperty("user.dir").replace('\\', '/') + "/conf/quartz.properties";
			Scheduler scheduler = new StdSchedulerFactory().getScheduler();
			scheduler.start();
			
			//添加同步数据库任务的排程
			String id = SynchronizedJob.class.getName();
			JobDetail jobDetail = new JobDetail(id,Scheduler.DEFAULT_GROUP,SynchronizedJob.class);
			String jobExpression = SystemConfig.getSystemConfig("khetlJobExpression");
			if(CommonUtil.isEmpty(jobExpression))
				jobExpression = "59 59 23 * * ?";
			
			Trigger trigger = new CronTrigger(id, "SynchronizedJob" ,jobExpression);
