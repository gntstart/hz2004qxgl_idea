...
var yhid=null;
var dqbm=null;
var flagsel = new Ext.data.SimpleStore({
	fields : ['name', 'flag'],  
	data : [['0', '民警'], ['1', '协警']]
});
...
function getzp(yhid,dqbm){
	url = 'util/upfile?type=xjzp&yhid=' + yhid +'&dqbm=' +dqbm+ '&flag=1' + '&_dc=' + (new Date()).getTime();
    document.getElementById("img_xj1").src = url;
}

function getzw(yhid,dqbm){
	url = 'util/upfile?type=xjzp&yhid=' + yhid +'&dqbm=' +dqbm+ '&flag=2' + '&_dc=' + (new Date()).getTime();
    document.getElementById("img_xj2").src = url;
}

function getzp_mj(yhid,dqbm){
	
	url = 'util/upfile?type=xjzp&yhid=' + yhid +'&dqbm=' +dqbm+ '&flag=1' + '&_dc=' + (new Date()).getTime();
    document.getElementById("img_mj1").src = url;
}

function getzw_mj(yhid,dqbm){
	url = 'util/upfile?type=xjzp&yhid=' + yhid +'&dqbm=' +dqbm+ '&flag=2' + '&_dc=' + (new Date()).getTime();
    document.getElementById("img_mj2").src = url;
}
...
function save(){
	document.getElementById("img_mj1").src ="";
	document.getElementById("img_mj2").src ="";
    ...
    var gmsfhm=Ext.getCmp("_gmsfhm_").getValue();
    var yhxm=Ext.getCmp("_xm_").getValue();
    var jyh=Ext.getCmp("_jh_").getValue();
	if(!isIDCard2(gmsfhm)){
	...
	rymxFs.form.submit({  
       	url:"util/upfile?className=zzjyManagerService&method=saveZzjy",
       	method:'POST',
       	params:{gmsfhm:gmsfhm,yhxm:yhxm,jyh:jyh},
       	//waitMsg: '正在执行操作，请等待...',
       	success:function(form,action){
             Ext.Msg.hide();
			//var jsonData1 = Ext.util.JSON.decode(action.response.responseText);
			...
			if(jsonData.success){
				...
				var data = Ext.util.JSON.decode(jsonData.result);		
				var r= ryCommonStore.getById(data.yhid);
				dqbm=data.dwdm.substring(0,4);
				if(r){
					getzp_mj(data.yhid,dqbm);
					getzw_mj(data.yhid,dqbm);
					for(var pname in data){
						r.set(pname,data[pname]);
					}
				}else{				
					...
					ryCommonStore.insert(0, [r1]);
					curRy = r1;
					dqbm=r1.data.dwdm.substring(0,4);
					editry(r1.data.yhid,dqbm);
					rymxFs.getForm().setValues(data);
					
					tabwin.setTitle("修改人员资料");
					
					Ext.getCmp("gridip_xj").setDisabled(false);
					Ext.getCmp("gridsjfw_xj").setDisabled(false);
					Ext.getCmp("gridtdqx_xj").setDisabled(false);
					Ext.getCmp("gridddqx_xj").setDisabled(false);
				}
...
function savexj(){
	if(!rymxFs_xj.getForm().isValid()){
		Ext.Msg.alert("提示","数据校验没有通过，请检查！");
		return;
	}
	
	var data = rymxFs_xj.getForm().getValues();
		...
		success:function(result,request){ 
			...
			if(jsonData.success){
				...
				if(r){
					...
					editxj(r1.data.yhid);
					rymxFs_xj.getForm().setValues(data);
					tabwin_xj.setTitle("修改人员资料");
...
function editxj(yhid,dqbm){
	...
	if(yhid!=""){
		...
		saveusid = yhid;
		Ext.getCmp("saveBtn_xj").setDisabled(true);
		Ext.getCmp("sqBtn_xj").setDisabled(true);
		Ext.getCmp("saveDwBtn_xj").setDisabled(true);
		if(jy_xm_edit=="0"){
			Ext.getCmp("_xm_xj").setDisabled(true);
			//Ext.getCmp("_jh_").setDisabled(true);
		}else{
			Ext.getCmp("_xm_xj").setDisabled(false);
		...
		rymxFs_xj.getForm().setValues({dwmc:jg.mc,dwdm: jg.dm});
		tabwin_xj.setTitle("新增协警");
	}
	tabwin_xj.show();
...
function editry(yhid,dqbm){
	var obj = document.getElementById("file3");  
	obj.outerHTML = obj.outerHTML;
	
	var obj2 = document.getElementById("file4");  
	obj2.outerHTML = obj2.outerHTML;
	...
	if(yhid!=""){
		...
		//if(jy_xm_edit=="0"){
			...
			Ext.getCmp("_jh_").setDisabled(true);
			Ext.getCmp("_gmsfhm_").setDisabled(true);
...
Ext.onReady( function() {
	...
	/********************************人员查询和列表*******************************/
 	ryCommonStore2 = new Ext.data.JsonStore({
	        ...
	        fields: [
				"yhid","uid","yhdlm","jyh","dwdm","yhxm","yhxb","yhzw","dlkl","yhmj","czmj","yhzt","gmsfhm","dwmc","lxdh","zwmc","zp","zw"
	        ],
	        listeners:{
	        	load:function(mystore,res){
	        		...
	        		if(res.length>0){
	        			...
	        			Ext.getCmp("saveBtn_xj").setDisabled(false);
	        			Ext.getCmp("sqBtn_xj").setDisabled(false);
	        			Ext.getCmp("saveDwBtn_xj").setDisabled(false);
	        			rymxFs.getForm().setValues(res[0].data);
	        			rymxFs_xj.getForm().setValues(res[0].data);
	        			...
	        			if(data.czmj){
	        				for(var i=0;i<data.czmj.length;i++){
	        					...
	        					if(val=="1"){
	        						...
	        						Ext.getCmp("czmj" + (i+1)+"_xj").setValue(true);
	        					}
	        				}
	        			}
	        		}
 	...
 	ryCommonStore = new Ext.data.JsonStore({
	        ...
	        id:'yhid',
	        totalProperty:'totalCount',
	        fields: ["yhid","uid","yhdlm","jyh","dwdm","yhxm","yhxb","yhzw","dlkl","yhmj","czmj","yhzt","gmsfhm","dwmc","lxdh","zwmc","flag","dqbm"],
	        listeners:{
				loadexception:function(mystore,options,response,error){
					if(error){
						var json = Ext.decode(response.responseText);
	...
	var ryColModel = new Ext.grid.ColumnModel([
		...
		width: 80
	        ...
	        renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
				if(value=="0") return "民警";
				if(value=="1") return "协警";
			}
	    },{
	        header: "用户状态",
	        dataIndex: "yhzt",
		...
		buttons:[
			...
			new Ext.Button({
				...
				handler:function(){
					...
					var obj2 = document.getElementById("file2");  
					document.getElementById("img_xj1").src ="";
					document.getElementById("img_xj2").src ="";
					obj2.outerHTML = obj2.outerHTML;
					if(selnode==null || !selnode.attributes.dw){
						Ext.Msg.alert("提示","新增人员，必须先在右边树中选择地区单位！");
						return;
	...
	var ryGrid = new Ext.grid.GridPanel({
		...
		listeners:{
			...
			rowdblclick:function(g, rowIndex, e){
				...
				jhbz=curRy.data.flag;
				dqbm=curRy.data.dwdm.substring(0,4);
				if(jhbz=="0"){
					editry(curRy.data.yhid,dqbm);
				}else{
					editxj(curRy.data.yhid,dqbm);
				}
	...
	rymxFs_xj = new Ext.form.FormPanel({
    	height: 100,
    	region: 'north',
    	anchor:'100% 100%',
    	buttonAlign:'right',
					...
					listeners:{
						blur:function(f){
							...
							if(sfz && sfz!=""){
								...
								rymxFs_xj.getForm().setValues({yhxb:xb});
							}
								...
								Ext.Ajax.request({
									...
									success:function(result,request){ 
										...
										if(jsonData.success && jsonData.list.length==1){
											var data = jsonData.list[0];
											if(data!=null){
												var newdata = {yhdlm:data.yhm,yhxm:data.yhm};
												rymxFs_xj.getForm().setValues(newdata);
											}
    ...
    height:160,
   	...
   	html:'<font style="font-size:9pt">&nbsp;照片：&nbsp;</FONT><img id="img_xj1" src=""   width=150 height=150></img>'
},{
    columnWidth:.5,
    height:160,
   	layout: 'form',
   	bodyStyle:'padding:5 0 0 15',
   	html:'<font style="font-size:9pt">&nbsp;指纹：&nbsp;</FONT><img id="img_xj2" src=""   width=150 height=150></img>'
}
		]},{
			xtype:'checkboxgroup',
			...
			columns: 1,
			items:[ 
	        		{boxLabel: '系统管理员', height:22, name: 'czmj1', id:'czmj1_xj', inputValue:"1"},
	            	{boxLabel: '上报数据专管员', height:22,name: 'czmj2', id:'czmj2_xj',inputValue:"2"},
	            	{boxLabel: '制证专管员', height:22,name: 'czmj3', id:'czmj3_xj',inputValue:"3"},
	            	{boxLabel: '一般用户', height:22,name: 'czmj4', id:'czmj4_xj',inputValue:"4",checked:true}
			]
		}
	],
	buttons:[
	...
	rymxFs = new Ext.form.FormPanel({
        ...
        height:160,
       	layout: 'form',
       	bodyStyle:'padding:5 0 0 15',
       	html:'<font style="font-size:9pt">&nbsp;照片：&nbsp;</FONT><img id="img_mj1" src=""   width=150 height=150></img>'
	},{
        columnWidth:.5,
        height:160,
       	layout: 'form',
       	bodyStyle:'padding:5 0 0 15',
       	html:'<font style="font-size:9pt">&nbsp;指纹：&nbsp;</FONT><img id="img_mj2" src=""   width=150 height=150></img>'
	}
			]},{
				xtype:'checkboxgroup',
				/*form布局，这里不能使用100%，否则看不到复选项，此外也必须保持
				...
				columns: 1,
				items:[ 
		        		{boxLabel: '系统管理员', height:22, name: 'czmj1',id:'czmj1',  inputValue:"1"},
		            	{boxLabel: '上报数据专管员', height:22,name: 'czmj2', id:'czmj2',inputValue:"2"},
		            	{boxLabel: '制证专管员', height:22,name: 'czmj3',id:'czmj3', inputValue:"3"},
		            	{boxLabel: '一般用户', height:22,name: 'czmj4',id:'czmj4', inputValue:"4",checked:true}
				]
			}
		],
		buttons:[
	...
	var gridIP_xj = new Ext.grid.GridPanel({
        store: IPStore,
        region: 'center',
        view:new Ext.grid.GridView({
			forceFit:true,
	...
	var gridSJFW_xj = new Ext.grid.GridPanel({
        store: SJFWStore,
        region: 'center',
        view:new Ext.grid.GridView({
			forceFit:true,
	...
	var gridTDQX_xj = new Ext.grid.GridPanel({
        store: TDQXStore,
        region: 'center',
        view:new Ext.grid.GridView({
			forceFit:true,
	...
	var gridDDQX_xj = new Ext.grid.GridPanel({
        store: DDQXStore,
        region: 'center',
        view:new Ext.grid.GridView({
			forceFit:true,
	...
	var tabs = new Ext.TabPanel({
	        ...
	        defaults:{
	        	//autoScroll: true,
	        	bodyStyle:'overflow-y:auto;height:260px',
	        	frame: false,
	        	shim: true
	        },
	        listeners:{
	...
	var tabs_xj = new Ext.TabPanel({
        ...
        listeners:{
        	tabchange:function(tabPanel , tab){
        		if(tab.id=="gridip_xj" && !initData.gridip_xj){
        			initData.gridip_xj = "1";
        			loadDataIP();
        		}
        		
        		if(tab.id=="gridsjfw_xj" && !initData.gridsjfw_xj){
        			initData.gridsjfw_xj = "1";
        			loadDataSJFW();
        		}
        		
        		if(tab.id=="gridtdqx_xj" && !initData.gridtdqx_xj){
        			initData.gridtdqx_xj = "1";
        			loadDataDTQX();
        		}
        		
        		if(tab.id=="gridddqx_xj" && !initData.gridddqx_xj){
        			initData.gridddqx_xj = "1";
        			loadDataDDQX();
        		}
        	} 
        },
        items:[
        	rymxFs_xj, gridIP_xj, gridSJFW_xj, gridTDQX_xj, gridDDQX_xj
	]
});
		...
		listeners:{
			show:function(){
				if(!isnewry){
					...
					Ext.getCmp("gridip_xj").setDisabled(true);
					Ext.getCmp("gridsjfw_xj").setDisabled(true);
					Ext.getCmp("gridtdqx_xj").setDisabled(true);
					Ext.getCmp("gridddqx_xj").setDisabled(true);
				}
				
				if(!curRy){
					Ext.getCmp("gridip_xj").store.removeAll();
					Ext.getCmp("gridsjfw_xj").store.removeAll();
					Ext.getCmp("gridtdqx_xj").store.removeAll();
					Ext.getCmp("gridddqx_xj").store.removeAll();
					tabs_xj.setActiveTab(0);
    		...
    		treeSelectWin.items.first().loadData(
    			...
    			function(idlist,list){
    				if(list.length==1){
						...
						rymxFs_xj.getForm().setValues({dwmc:list[0].mc,dwdm:list[0].dm});
    				}
    			}
    		);
	}
