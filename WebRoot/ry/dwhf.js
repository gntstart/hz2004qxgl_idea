...
Ext.onReady( function() {
	...
	t_dataUrl='jgryhf.do?method=queryZzjgTree';
	if(window._isAdmin && window._isAdmin!="1"){
		t_dataUrl='jgryhf.do?method=queryZzjgTree&sjfw=' + window._sjfw;
		t_sjfw=window._sjfw;
	}
	...
	projectTreeLoader.on("beforeload", function(treeLoader, node) {
			...
			if(node.attributes.code){
				this.dataUrl ="jgryhf.do?method=queryZzjgTree&organizeBm=" + node.attributes.code.code;
	    }
		}, 
		projectTreeLoader
	);
	...
	projectTree.on("click", function(node,e) {
			if(node.attributes.code){
				...
				fnQueryFs.setTitle("已删除单位列表【"+node.attributes.code.name+"】");
	...
	fnCommonStore = new Ext.data.JsonStore({
        url: 'jgryhf.do?method=queryZzjg',
        root: 'list',
        id:'usid',
		  		...
		  		if(btn=="yes"){
		 			var ids = "";
		   			for(var i=0;i<res.length;i++)
		   				ids += res[i].data.orgid + ",";
					
					//恢复
					Ext.Msg.wait("正在恢复选择的单位，请稍后...", "等待");
					
					Ext.Ajax.request({
						url:'jgryhf.do?method=recoverDw',
						method:'POST', 
						params:{ids:ids},
