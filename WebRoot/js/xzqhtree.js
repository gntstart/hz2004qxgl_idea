	...
	var _XZQHTree = new Ext.tree.TreePanel({
	   	...
	   	listeners		:{
	    		...
	    		},beforeexpandnode:function(node,checked){
	...
	var _XZQHWin = new Ext.Window({
		...
		buttons:[
			new Ext.Button({
				...
				handler:function(){
					//var node = null;
					var nodeArray=new Array(_XZQHTree.getChecked().length);
					Ext.each(_XZQHTree.getChecked(),function(item,index){
					...
					for(var i=0;i<nodeArray.length;i++){
						...
						if(nodeArray[i].attributes.jwh){
							sjfw += nodeArray[i].attributes.jwh.dm+",";
						}else if(nodeArray[i].attributes.xzqh){
							sjfw += nodeArray[i].attributes.xzqh.dm+",";
						}else if(nodeArray[i].attributes.dw){
							sjfw += nodeArray[i].attributes.dw.dm+",";
						}
					}
