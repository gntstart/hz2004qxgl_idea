...
public class DQDictDataShare {
	...
	static public final Map<String,XT_XZQHB> ahMap = new HashMap<String,XT_XZQHB>();
	static public final Map<String,XtDwxxb> pcsMap = new HashMap<String,XtDwxxb>();
	//行政区划树结构，省库获取
	static public final Map<String,List<XT_XZQHB>> xzqhTreeData = new HashMap<String,List<XT_XZQHB>>();
	static public final Map<String,List<XT_XZQHB>> ahTreeData = new HashMap<String,List<XT_XZQHB>>();
	static public final Map<String,List<XtDwxxb>> pcsTreeData = new HashMap<String,List<XtDwxxb>>();
	 ...
	 * 重载安徽省行政区划
	 */
	static public void realoadXzqh1(){
			...
			synchronized(ahMap){
				if(ahMap.size()==0){
					...
					try{
						...
						stmt = conn.createStatement();
						String sql = "select * from XT_XZQHB" + 
										" where 1 = 1 " +
										" and (qybz = '1' or mc = '安徽省')" +
//										" and mc like '%安徽省%' " +
										" and dm like '34%' " +
										" order by dm";
						res = stmt.executeQuery(sql);
						Map<String,List<XT_XZQHB>> ahTreeData_tmp = new HashMap<String,List<XT_XZQHB>>();
						while(res.next()){
							XT_XZQHB qh = ObjectUtil.copyInfo(XT_XZQHB.class, null, res);
							ahMap.put(qh.getDm(), qh);
							...
							List<XT_XZQHB> list = ahTreeData_tmp.get(parentKey);
							if(list==null){
								list = new ArrayList<XT_XZQHB>();
								ahTreeData_tmp.put(parentKey,list);
							}
							
							list.add(qh);
						}
						
						ahTreeData.putAll(ahTreeData_tmp);
						ahTreeData_tmp.clear();
					}catch(Exception ex){
						ex.printStackTrace();
						throw new ServiceException(ex);
					}finally{
			...
			synchronized(pcsMap){
				if(pcsMap.size()==0){
						...
						try{
							...
							stmt = conn.createStatement();
							res = stmt.executeQuery("select * from Xt_Dwxxb order by dm");
							Map<String,List<XtDwxxb>> pcsTreeData_tmp = new HashMap<String,List<XtDwxxb>>();
							while(res.next()){
								XtDwxxb pcs = ObjectUtil.copyInfo(XtDwxxb.class, null, res);
								pcsMap.put(pcs.getDm(), pcs);
								
								String parentKey = pcs.getDm();
								if(!parentKey.endsWith("000")){
									parentKey = parentKey.substring(0, 6) + "000";
								}else{
									parentKey = parentKey.substring(0, 6);
								}
								
								List<XtDwxxb> list = pcsTreeData_tmp.get(parentKey);
								if(list==null){
									list = new ArrayList<XtDwxxb>();
									pcsTreeData_tmp.put(parentKey,list);
								}
								
								list.add(pcs);
							}
							
							pcsTreeData.putAll(pcsTreeData_tmp);
							pcsTreeData_tmp.clear();
						}catch(Exception ex){
							ex.printStackTrace();
							throw new ServiceException(ex);
						}finally{
	...
	static public List<TreeNode> getTreeXzqh1(String parent_dm){
		List<TreeNode> list = new ArrayList<TreeNode>();
		if(ahMap.size()==0){
			realoadXzqh1();
		}
		...
		List<XT_XZQHB> childs = ahTreeData.get(parent_dm);
		if(childs != null && childs.size() > 0){
			for(XT_XZQHB child:childs){
				...
				List<XtDwxxb> pcsChilds = pcsTreeData.get(child.getDm());
				List<XT_XZQHB> childsc = ahTreeData.get(child.getDm());
				if((pcsChilds != null && pcsChilds.size() > 0 )
					|| (childsc != null && childsc.size() > 0)){
					node.setLeaf(false);
				}else{
					node.setLeaf(true);
				}
			...
			List<XtDwxxb> pcsChilds = pcsTreeData.get(parent_dm);
			if(pcsChilds != null && pcsChilds.size() > 0){
				for(XtDwxxb child:pcsChilds){
					TreeNode node = new TreeNode();
					node.setChecked(false);
					node.setText(child.getMc());
					node.setLeaf(!child.getDm().endsWith("00"));
					XT_XZQHB qh = new XT_XZQHB();
					qh.setDm(child.getDm());
					node.setXzqh(qh);
					node.setCodevalue(child.getDm());
