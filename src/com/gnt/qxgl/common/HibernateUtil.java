...
import com.gnt.qxgl.service.impl.ZzjyManagerServiceImpl;
...
public class HibernateUtil {
	...
	public synchronized  static SessionFactory getSystemSessionFactory(String jndi)
		...
		try {
			...
			if(!ZzjyManagerServiceImpl.dwMap.containsKey(jndi)){
				//如果没有加载单位，那么加载单位
				Session session = null;
				try{
					session = HibernateUtil.getSystemSessionFactory(jndi).openSession();
					List list = HibernateUtil.getObjectList(session, "from XtDwxxb", null);
					
					Map<String,String> map = new HashMap<String,String>();
					for(Object obj:list){
						XtDwxxb dw = (XtDwxxb)obj;
						map.put(dw.getDm(), dw.getMc());
					}
					
					ZzjyManagerServiceImpl.dwMap.put(jndi, map);
				}catch(Exception e){
					if(session!=null)
						session.close();
				}
	...
	public synchronized  static SessionFactory getSystemSessionFactory2(String jndi)
			throws HibernateException {
		if(jndi==null)
			jndi = "prodb";
		...
		String cfgFile = "/conf/prodb/" + jndi + ".cfg.xml";
		SessionFactory sf = null;
		try {
			Configuration conf  = new Configuration().configure(cfgFile);
			sf = conf.buildSessionFactory();
			...
			if(!ZzjyManagerServiceImpl.dwMap.containsKey(jndi)){
				...
				Session session = null;
				try{
					session = HibernateUtil.getSystemSessionFactory2(jndi).openSession();
					List list = HibernateUtil.getObjectList(session, "from XtDwxxb", null);
	...
	static public long getMaxSeq2(Session session, String pojoname,String pname,String dwdm){
		String dqbm = dwdm.substring(0,4);
		
		String hql = null;
		
		hql = "select max(a." + pname + ") from " + pojoname + " a where a.yhid like '"+dqbm+"%'";
	...
	static public List<?> executeSqlQuery(Session session, final String sql, final Object[] param) {
