	...
	public List<TreeNode> queryXZQHTreeNodes(Map<String,String> param){
		...
		try{
			...
			if(CommonUtil.isEmpty(xzqh) && CommonUtil.isEmpty(pcs)){
				...
				//为空，那么省级行政区划
				String sql = "from XT_XZQHB where dm like '%0000'  order by dm";
				List l = HibernateUtil.getObjectList(session, sql, null);
				for(Object obj:l){
					XT_XZQHB qh = (XT_XZQHB)obj;
					if(qh.getDm().substring(0, 2).equals(dwdm)&&qh.getQybz().equals("1")){
						TreeNode node = new TreeNode();
						node.setXzqh(qh);
						node.setChecked(new Boolean(false));
			...
			if(CommonUtil.isNotEmpty(xzqh)){
				if(xzqh.endsWith("0000") || xzqh.endsWith("00")){
					...
					if(xzqh.endsWith("0000")){
						sql = "from XT_XZQHB where dm like '" + xzqh.substring(0,2) + "%00' and dm<>'" + xzqh + "' and qybz='1' order by dm";
					}else{
						sql = "from XT_XZQHB where dm like '" + xzqh.substring(0,4) + "%' and dm<>'" + xzqh + "' and qybz='1' order by dm";
					}
