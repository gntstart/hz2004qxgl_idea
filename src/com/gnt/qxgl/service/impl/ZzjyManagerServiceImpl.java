...
import com.gnt.qxgl.hz2004.entity.XtYhZpxxb;
...
public class ZzjyManagerServiceImpl extends ServiceImpl implements ZzjyManagerService{
	...
	public SimpleJson saveZzjy(Map parm, HttpServletRequest req) {
		//3413000001000000495
		//"yhid","uid","yhdlm","jyh","dwdm","yhxm","yhxb","yhzw","dlkl","yhmj","czmj","yhzt","gmsfhm","dwmc"
		String yhid = (String) parm.get("yhid");
		String yhdlm = (String) parm.get("yhdlm");
		String jyh = (String) parm.get("jyh");
		String dwdm = (String) parm.get("dwdm");
		String yhxb = (String) parm.get("yhxb");
		String yhzw = (String) parm.get("yhzw");
		String dlkl = (String) parm.get("dlkl");
		String yhmj = (String) parm.get("yhmj");
		String yhzt = (String) parm.get("yhzt");
		String yhxm = (String) parm.get("yhxm");
		String lxdh = (String) parm.get("lxdh");
		String flag = (String) parm.get("jybz");
		FileItem zp = (FileItem)parm.get("zp");
		
		FileItem zw = (FileItem)parm.get("zw");
	    
		if(CommonUtil.isEmpty(yhzt))
			yhzt = "0";
		
		String gmsfhm = (String) parm.get("gmsfhm");
		String editpwd = (String) parm.get("editpwd");
		
		String czmj = "";
		//czmj4
		for(int i=1;i<=4;i++){
			String key = "czmj" + i;
			String val=String.valueOf(parm.get(key));
			if(("null").equals(val)){
				czmj += "0";
			}else{
				czmj += "1";
		...
		XtYhxxb yh = null;
		XtYhZpxxb yhzp=null;
		InputStream inputzw=null;
		InputStream inputzp=null;
		boolean isnew = true;
		boolean isnewzp = true;
		try{
			session = HibernateUtil.getSystemSessionFactory(dqbm).openSession();
			tran = session.beginTransaction();
			...
			if(CommonUtil.isEmpty(yhid)){
				...
				//新增，需要创建一个ID
				long seq = HibernateUtil.getMaxSeq2(session, "XtYhxxb", "yhid", dwdm);
				...
				yhzp=new XtYhZpxxb();
				long yhzp_seq = HibernateUtil.getMaxSeq(session, "XtYhZpxxb", "zpid", dwdm);
				yhzp.setZpid(String.valueOf(yhzp_seq));
				yhzp.setYhid(String.valueOf(seq));
				...
				if(CommonUtil.isNotEmpty(editpwd))
					yh.setDlkl(dlkl);
				List<XtYhZpxxb> li=HibernateUtil.getObjectList(session, "from XtYhZpxxb where yhid=?", new Object[]{yhid});
			...
			if(zp!=null && zp.getSize()>0){
				String fname = zp.getName();
				if(fname.indexOf(".")>0)
					fname = fname.substring(fname.lastIndexOf(".")+1);
				...
				//if(zp.getSize()>20480000)
				//	throw new ServiceException("错误，上传的照片文件太大，最多不能超过2M！");
				
				 inputzp = new BufferedInputStream(zp.getInputStream());		   
				 Blob zpimage = Hibernate.createBlob(inputzp,inputzp.available());
				 yhzp.setZp(zpimage);
				
			}
			
			if(zw!=null && zw.getSize()>0){
				String zwname = zw.getName();
				if(zwname.indexOf(".")>0)
					zwname = zwname.substring(zwname.lastIndexOf(".")+1);
				
				if ("jpg,jpeg,gif,png,bmp,JPG,JPEG,GIF,PNG,BMP".indexOf(zwname)<0)
					throw new ServiceException("上传的照片必须为图片文件！");
				
				//if(zw.getSize()>20480000)
				//	throw new ServiceException("错误，上传的照片文件太大，最多不能超过2M！");
	
				inputzw = new BufferedInputStream(zw.getInputStream());		   
			    Blob zwimage = Hibernate.createBlob(inputzw,inputzw.available());
			...
			if(isnew){
				...
				HibernateUtil.create(session, yhzp);
			}else{
				HibernateUtil.update(session, yh);
				if(isnewzp){
					HibernateUtil.create(session, yhzp);
				}else{
					HibernateUtil.update(session, yhzp);
				}
		...
		SimpleJson s=savaProdb(yh,yhzp,flag,isnewzp,isnew,zw,zp,dwdm);
	 ...
	 * @return
	 */
	public SimpleJson savaProdb(XtYhxxb yh,XtYhZpxxb yhzp,String flag,Boolean isnewzp,Boolean isnew,FileItem zw,FileItem zp,String dwdm){
		Session session = null;
		Transaction tran = null;
		InputStream inputzw=null;
		InputStream inputzp=null;
		try {
			//地市已经保存，只需要到省厅保存下就好了
			session = HibernateUtil.getSystemSessionFactory2("prodb").openSession();
			
			tran = session.beginTransaction();
			//XtYhxxb yhpo = HibernateUtil.get(session, XtYhxxb.class, yh.getYhid());
			//BeanUtils.copyProperties(yhpo, yh);
		//	if(null!=yhpo){
		...
		String strJson=JSONObject.fromObject(yh).toString();
		try {
			s.setResult(strJson);
		} catch (Exception e) {
			// TODO: handle exception
		...
		if(isnew){
			s.setMessage("新增"+flag+"用户成功!");
		}else{
			s.setMessage("修改"+flag+"用户成功!");
		}
	...
	public List createSJFW(Map parm){
		...
		try{
			...
			if(sjfw.length()>9){
				...
				sjfw = (sjfw.substring(0, 6) + "|" + sjfw).replace(",", "|");
			  ...
			  for(int i=0;i<array.length;i++){
				  ...
				  if(array[i].length()>9){
					  sjfw=array[i].substring(0, 6)+ "|" +array[i].substring(0, 9)+"|"+array[i]+"|";
				  }
				  else if(array[i].length()==9){
					  sjfw=array[i].substring(0, 6)+ "|" +array[i]+"||";
				  }else{
					  sjfw = array[i] + "|||";
				  }
	...
	public List<TreeNode> queryDwxxTree(HttpServletRequest req) {
		...
		if(CommonUtil.isEmpty(postDetail)){
			if(loginu.getSsdwjgdm().startsWith("3400")){
				...
				String hql = "from XtDwxxb where dwjgdm like '%00000000' and qybz='1' order by dwjgdm asc";
				...
				//地市，只能看到自己地市节点
				String hql = "from XtDwxxb where dwjgdm = '" + loginu.getSsdwjgdm().substring(0,4) + "00000000' and qybz='1' order by dwjgdm asc";
				List<?> list = super.getObjectListByHql(hql);
				for(Object o:list){
					XtDwxxb dw = (XtDwxxb)o;
					TreeNode node = new TreeNode();
			...
			String hql = "from XtDwxxb where qybz='1' and ";
	...
	public List<TreeNode> queryDwxxTreeWin(HttpServletRequest req) {
		XtYh loginu = BaseContext.getBaseUser().getUser();
		if(loginu==null)
			throw new ServiceException("错误，你的登录已经超时，请重新登陆！");
	...
	public void SynchronizedKHZB(){
		String hql = "from XtYhksjg where dwdm not like '3400%' and  dwdm not like '0000%' order by dwdm asc";
		List<?> list = super.getObjectListByHql(hql);
	...
	@Override
	public byte[] getzzjyZp(String yhid,String dqbm) {
		Session session = null;
		Transaction tran = null;
		byte [] zp=null;
		if(null!=dqbm&&!dqbm.equals("")&&!dqbm.equals("null")){
		try{
			session = HibernateUtil.getSystemSessionFactory(dqbm).openSession();
			tran = session.beginTransaction();
			
		
				if(CommonUtil.isNotEmpty(yhid)){
					String sql = "select zp from xt_yhzpxxb a where yhid=?";
					Object[] pa=new Object[1];
					pa[0]=yhid;
					List li=HibernateUtil.executeSqlQuery(session, sql,pa);
					if(li!=null&&li.size()>0){
						Blob img=(Blob) li.get(0);
						if(null!=img){
							zp=img.getBytes(1, (int) img.length());
						}
	...
	@Override
	public byte[] getzzjyZw(String yhid, String dqbm) {
		Session session = null;
		Transaction tran = null;
		byte [] zp=null;
		if(null!=dqbm&&!dqbm.equals("")&&!dqbm.equals("null")){
			try{
				...
				tran = session.beginTransaction();
					...
					if(CommonUtil.isNotEmpty(yhid)){
						String sql = "select zw from xt_yhzpxxb a where yhid=?";
						Object[] pa=new Object[1];
						pa[0]=yhid;
						List li=HibernateUtil.executeSqlQuery(session, sql,pa);
						if(li!=null&&li.size()>0){
